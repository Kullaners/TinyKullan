...
def _write_csv_macro(events, filepath):
    ...
    for ev in events:
        ...
        content.append(f"{t},{d},{x},{y},{btn},{up},{vk},{scan},{ext},{delta}")
...
class Config:
    ...
    def load(self):
        ...
        try:
            ...
            pass
        self.tiny_mode = b("UI", "TinyMode", True)
...
class App:
    ...
    def _start_recording(self):
        ...
        try:
            ...
            macro_temp = os.path.join(script_dir, "temp_macro.txt")
    ...
    def _ahk_playback_worker(self, loop):
        ...
        try:
            ...
            # Write events to temp CSV file
            _write_csv_macro(self.events, macro_temp)
...
{me.JS_SCRIPT}
    ...
    def _open_macro_editor(self):
        ...
        class VirtualEventList:
            ...
            def _on_click(self, e):
                ...
                if ctrl: self.select_toggle(idx); self._drag_idx = idx; self._drag_y = float(e.y_root)
            ...
            def _do_pen_click(self, idx, ev):
                ...
                ans = simpledialog.askstring("Rename Action",
                    ...
                    parent=self.outer.winfo_toplevel())
        ...
        def _on_reorder(src_idx, dst_idx):
            save_state()
            with self._ev_lock:
                if 0 <= src_idx < len(self.events) and 0 <= dst_idx < len(self.events):
                    self.events[src_idx], self.events[dst_idx] = \
                        self.events[dst_idx], self.events[src_idx]
                            ...
                            if max_val >= 0.55:
                                ...
                                if action == "click":
                                    ...
                                    for ox, oy in offsets:
                                        ...
                                        time.sleep(0.05)
                                        _send_input(_mouse_click(tx, ty, "left", False))
                                        time.sleep(0.06)
                                        _send_input(_mouse_click(tx, ty, "left", True))
                                        time.sleep(0.08)
                                    pt3 = POINT()
    ...
    def _release_held(self):
        ...
        for btn_name in list(self._held_btns):
            try:
                _send_input(_mouse_button(btn_name, True))
            except Exception:
                pass
    ...
    def _image_search_worker(self):
        ...
        while True:
            try:
                ...
                _LOG.error("Error in image search worker: %s", e)
            time.sleep(0.1)

    def _find_best_match(self, screen_bgr, template_bgr):
        import cv2
        import numpy as np
    ...
    def _handle_roblox_recovery(self, screen_bgr):
        ...
        import cv2
        template = self._template_cache.get(img_path)
        if template is None:
            try:
                template = cv2.imread(img_path, cv2.IMREAD_COLOR)
                if template is not None:
                ...
                _LOG.error("Failed to load Roblox recovery image: %s", e)
                return
        if template is None:
            return
        ...
        # ── Set flag: tells the worker thread not to run cleanup ─────────
        ...
        try:
            ...
            # Stop AHK. Worker sees _recovering=True and skips cleanup.
            if self._ahk_proc is not None:
                try:
                    self._ahk_proc.terminate()
                except Exception:
    ...
    def _check_images(self):
        ...
        if getattr(self.cfg, "roblox_enabled", False):
            try:
                self._handle_roblox_recovery(screen_bgr)
            except Exception as e:
                _LOG.error("Error in Roblox recovery: %s", e)
        ...
        for tgt in targets:
            ...
            if action == "click" and img_path in getattr(
                self, "_clicked_images", set()
            ):
                continue
            ...
            if template is None:
                try:
                    ...
                    _LOG.error("Failed to load template %s: %s", tgt.get("name"), e)
                    continue
            ...
            if max_val >= threshold:
                ...
                if action == "click":
                    ...
                    # 1. Pause macro playback and get current cursor position
                    self._pause_playback = True
                    time.sleep(0.02)  # Give playback thread a tiny moment to pause
                    pt = POINT()
                    user32.GetCursorPos(_ct.byref(pt))
                    orig_x, orig_y = pt.x, pt.y

                    # Verification Loop: Keep clicking until the button disappears (Max 5 attempts)
                    for attempt in range(5):
                        # 2. 3-Point Box/Triangle Click Pattern
                        # Clicks 3 times in a triangular box around the center using absolute mouse-click inputs (like the recording).
                        offsets = [
                            (-5, -5),  # Top-Left
                            (5, -5),  # Top-Right
                            (0, 5),  # Bottom-Center
                        ...
                        for ox, oy in offsets:
                            ...
                            # 1. Humanized relative mouse movement to point (no teleport)
                            pt2 = POINT()
                            user32.GetCursorPos(_ct.byref(pt2))
                            dx2, dy2 = target_x - pt2.x, target_y - pt2.y
                            if dx2 != 0 or dy2 != 0:
                                _send_input(_mouse_move_rel(dx2, dy2))
                            time.sleep(0.05)  # Hover delay

                            # 2. Humanized Click Down (using _mouse_click like macro playback)
                            _send_input(_mouse_click(target_x, target_y, "left", False))
                            time.sleep(0.06)  # Human hold duration

                            # 3. Humanized Click Up (using _mouse_click like macro playback)
                            _send_input(_mouse_click(target_x, target_y, "left", True))
                            time.sleep(0.08)  # Settle duration between clicks

                        # 3. Verification Check
                        time.sleep(0.30)  # Wait 300ms for Roblox UI to visually update
                        try:
                            verify_screen = _grab_screen()
                            verify_np = np.array(verify_screen)
                            verify_bgr = cv2.cvtColor(verify_np, cv2.COLOR_RGB2BGR)

                            verify_val, _ = self._find_best_match(verify_bgr, template)
                            if verify_val < threshold:
                                # Success! The button disappeared from the screen
                                break
                            else:
                                _LOG.info(
                                    "Image Detection: Roblox ignored click (Conf: %.2f). Retrying %d/5...",
                                    verify_val,
                                    attempt + 1,
                                )
                                time.sleep(0.1)  # Brief pause before hammering it again
                        except Exception as e:
                            _LOG.error("Verification error: %s", e)
                            break

                    # Register click so it doesn't repeat until next macro run
                    if hasattr(self, "_clicked_images"):
                        self._clicked_images.add(img_path)

                    # 4. Move back to original position with relative move and resume playback
                    pt2 = POINT()
                    user32.GetCursorPos(_ct.byref(pt2))
                    dx2, dy2 = orig_x - pt2.x, orig_y - pt2.y
                    if dx2 != 0 or dy2 != 0:
                        _send_input(_mouse_move_rel(dx2, dy2))
                    self._pause_playback = False
                else:
    ...
    def _open_settings(self):
        ...
        def _add_image():
            ...
            try:
                ...
                _LOG.error("Failed to add target image: %s", e)
